...
<li>
    <button
            ...
            :class="isSelected($el.id) ? 'bg-gray-50 text-primary-500' : 'text-light-text-color'"
            {{ $attributes->merge(['class' => 'hover:bg-white group hover:text-neutral-dark rounded-md px-3 py-2 flex items-center']) }}
            role="tab"
    >
        <x-dynamic-component
                :component="'heroicon-o-'.$icon"
                :class="isSelected($el.parentElement.id) ? 'bg-gray-50 text-primary-500' : 'text-light-text-color'"
